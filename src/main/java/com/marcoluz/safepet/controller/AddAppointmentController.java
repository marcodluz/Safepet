...
public class AddAppointmentController implements Initializable {
  ...
  public void createAppointment(ActionEvent event) throws IOException {
    boolean appointmentPetValidatorNull = DataValidation.comboBoxNull(
      appointmentPet,
      appointmentPetError,
      "You need to select one pet"
    );
    boolean appointmentTypeValidatorNull = DataValidation.comboBoxNull(
      appointmentPet,
      appointmentTypeError,
      "You need to select one type"
    );

    if (
      !appointmentPetValidatorNull &&
      !appointmentTypeValidatorNull &&
      !isDatePast(
        String.valueOf(appointmentDatePicker.getValue()),
        "yyyy-MM-dd"
      )
    ) {
      ...
      String SQL =
        "INSERT INTO appointment (id, account_id, pet_id, type, status, date) values (?, ?, ?, ?, ?, ?);";
