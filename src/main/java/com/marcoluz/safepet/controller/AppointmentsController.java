...
public class AppointmentsController implements Initializable {
  ...
  public ObservableList<Appointment> getAppointmentList() throws SQLException {
    ...
    String SQL =
      "SELECT * FROM appointment WHERE (account_id = " +
      MainController.id +
      " AND date >= '" +
      localDate +
      "') ORDER BY date ASC";
  ...
  public void listAppointments() throws SQLException {
    ...
    clm_pet_id.setCellValueFactory(
      new PropertyValueFactory<Appointment, Integer>("pet_id")
    );
    clm_type.setCellValueFactory(
      new PropertyValueFactory<Appointment, String>("type")
    );
    clm_date.setCellValueFactory(
      new PropertyValueFactory<Appointment, String>("date")
    );
    clm_status.setCellValueFactory(
  ...
  public void listPastAppointments() throws SQLException {
    ...
    clm_past_pet_id.setCellValueFactory(
      new PropertyValueFactory<Appointment, Integer>("pet_id")
    );
    clm_past_type.setCellValueFactory(
      new PropertyValueFactory<Appointment, String>("type")
    );
    clm_past_date.setCellValueFactory(
      new PropertyValueFactory<Appointment, String>("date")
    );
    clm_past_status.setCellValueFactory(
      new PropertyValueFactory<Appointment, String>("status")
    );
  ...
  private void goToAddAppointment(ActionEvent event) throws IOException {
    AnchorPane pane = FXMLLoader.load(
      getClass().getResource("/com/marcoluz/safepet/add-appointment-page.fxml")
    );
    middleRootPane.getChildren().setAll(pane);
  }
}
