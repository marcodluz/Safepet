...
public class AccountDAO {
  ...
  public static boolean checkPassword(
    ...
    throws SQLException {
    if (
      getPassword(MainController.email, password.getText()) != 1
    ) {
  ...
  public static boolean checkEmailExists(
    ...
    throws SQLException {
    String SQL =
      "SELECT email FROM account WHERE (email = '" + email.getText() + "')";
    ResultSet rs = DBUtil.sqlExecute(SQL);
  ...
  public static String getLastName(String email, String password)
    throws SQLException {
    String SQL =
      "SELECT last_name FROM account WHERE (email = '" +
      email +
      "' AND  password = crypt('" +
      password +
      "', password)) LIMIT 1";
  ...
  public static String getEmail(String email) throws SQLException {
    String SQL =
      "SELECT email FROM account WHERE (email = '" + email + "') LIMIT 1";
    ResultSet rs = DBUtil.sqlExecute(SQL);
  ...
  public static int getPassword(String email, String password) throws SQLException {
